...
# =============== CONFIG ===============
URL = "https://www.dercocenter.cl/busqueda"  # <-- CAMBIA a la URL real del listado
HEADLESS = False
SLOWMO_MS = 0
VIEWPORT = {"width": 1400, "height": 950}

# Elige qué marcas recorrer (tal como aparecen en el label del filtro)
BRANDS = ["Suzuki", "GWM", "Renault","Changan","JAC","Deepal"]  # agrega o quita
...
def extract_money_from_h5(h5_locator) -> Optional[int]:
    """
    Devuelve el monto encontrado en un h5:
      - Si hay <span>, intenta allí.
      - Si no, busca el último número en el texto del h5.
    """
    try:
...
def extract_cards_from_grid(page, base_url: str) -> List[Dict]:
    ...
    for i in range(cards.count()):
        ...
        try:
            ...
            if card.locator(SEL_PRICES_BLOCK).count():
                ...
                # Además, intento un fallback específico si hay una estructura con clases
    ...
    # Dedupe por URL+versión (o por claves visibles si no hay URL)
    seen = set()
    out = []
    for r in data:
        if r.get("url_modelo"):
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        for brand in BRANDS:
            print(f"\n=== MARCA: {brand} ===")
            # marca on + expandir
            ensure_brand_checked(page, brand, True)
            ...
            for mv in modelos:
                ...
                cards = extract_cards_from_grid(page, base_url=URL)
                print(f"[OK] Tarjetas extraídas: {len(cards)}")
                for r in cards:
